...
- **[Global search (⌘K palette)](issues/global-search.md)** ([#17](https://github.com/evanhalley/beet/issues/17)) — make the inert "Search PRs, runs, repos…" pill in the TitleBar real. ⌘K opens a modal palette; client-side substring + initials matcher filters items already in `useAppStore`. Explicitly not a GitHub API search. Corpus widens as Standalone Runs / Recently Resolved / Mentions land in #6 / #8.
