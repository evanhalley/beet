...
| 4 | **My PRs + In Flight + 3-pane shell + lifecycle history** | Main window adopts the design's three-pane shell (sidebar / list / detail). "In Flight (Yours)" populates from `author:@me` search. `Lifecycle` pill renders, including `merge_queue · pos N`. Lifecycle transitions persist to SQLite. | `MainWindowShell` + `Sidebar` + `DetailPane`, selection state in Zustand, `pr_lifecycle_history` table, merge-queue ejection detection (unread bump only — no notification yet). | [issues/04-my-prs-in-flight.md](issues/04-my-prs-in-flight.md) |
